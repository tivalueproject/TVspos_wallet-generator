...
object WalletGenerator extends App {
  ...
  val AgentName = "TV Wallet Generator"
  val AgentVersion = "0.1.0"
  val AgentString = "TV Wallet Specification:" + WalletSpecification + "/" + AgentName + ":" + AgentVersion
  ...
  parser.parse(args, Config()) map { config =>
    ...
    val addrVersion:Byte = 29
    val chainId:Byte = if(config.testnet) 'T' else ';'
